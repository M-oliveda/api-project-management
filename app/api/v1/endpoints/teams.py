...
from app.schemas import TeamBase, TeamUpdate, Team, TeamWithMembers, TeamCreate, AddTeamMember, RemoveTeamMember
from app.services import create_team, get_team, update_team, delete_team, add_member_to_team, remove_member_from_team, get_team_by_owned_by
...
def add_new_team_endpoint(request: Request, team_data: TeamBase, db: Session = Depends(get_db)):
...
def get_team_endpoint(request: Request, team_id: UUID, db: Session = Depends(get_db)):
    ...
    return TeamWithMembers(id=team_id, name=team.name, owner_id=team.owner_id, members=[member.id for member in team.members])
...
def update_team_endpoint(request: Request, team_id: UUID, team_data: TeamUpdate, db: Session = Depends(get_db)):
    token = request.headers.get("Authorization")
    if not token or not verify_token(db, Token(access_token=token, token_type="bearer")):
        raise HTTPException(status_code=401, detail="Unauthorized")
...
def delete_team_endpoint(request: Request, team_id: UUID, db: Session = Depends(get_db)):
    token = request.headers.get("Authorization")
    if not token or not verify_token(db, Token(access_token=token, token_type="bearer")):
        raise HTTPException(status_code=401, detail="Unauthorized")
...
def add_member_endpoint(request: Request, add_team_member: AddTeamMember, db: Session = Depends(get_db)):
    token = request.headers.get("Authorization")
    if not token or not verify_token(db, Token(access_token=token, token_type="bearer")):
        raise HTTPException(status_code=401, detail="Unauthorized")
...
def remove_member_endpoint(request: Request, user_to_remove: RemoveTeamMember, db: Session = Depends(get_db)):
    token = request.headers.get("Authorization")
    if not token or not verify_token(db, Token(access_token=token, token_type="bearer")):
        raise HTTPException(status_code=401, detail="Unauthorized")
